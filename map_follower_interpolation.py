...
class MapFollower:
    def __init__(self):
        ...
        #############################################################################################    
        self.waypoints = Util.parseWaypoints('dudl_hallway.yaml')
        # self.waypoints = [[14.5,-1.3,0.0],[14.5,2.4,0.0]]
        self.index = 1
    ...
    def update_current_pose(self, msg) -> None:
        ...
        self.current_steering_angle = math.radians(yaw)
        self.last_pose_update_time = time.monotonic()
    ...
    def control_loop(self, waypoint_tolerance : float = 20, will_loop : bool = True) -> None:
        ...
        pose_interpolation = Util.Interpolation.pose_interp(self.current_pose, self.current_steering_angle, self.current_vel, self.last_pose_update_time)
        self.last_pose_update_time = pose_interpolation.pop()
        self.current_pose = pose_interpolation.pop()
        
        # theta_final = self.current_steering_angle - self.process_steering_target(self.previous_waypoint, self.desired_waypoint, self.current_pose, self.steering_vel)
        theta_final = Util.SteeringTarget.global_to_local_steering(self.current_steering_angle, Util.SteeringTarget.calculate_waypoint_angle(self.current_pose, self.desired_waypoint))
        # rospy.loginfo("Theta_final : {0}".format(theta_final))
        ...
        self.vel_error =  self.accel_PID(self.current_vel - 3)
        self.steer_error = self.steering_PID(theta_final)
    ...
    def write_accel(self) -> None:
        ...
        target_vel = Util.clamp((target_vel * 100), max=2048, min=-2048)
...
def main():
    ...
    while not rospy.is_shutdown():
        map_follower_instance.control_loop(waypoint_tolerance=.5, will_loop=True)
